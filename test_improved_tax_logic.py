...
def test_improved_calculations():
    ...
    print(
        f"  Standard Deduction Used: ${result.get('standard_deduction_used', 0):,.2f}"
    )
    ...
    result = tax_engine.calculate_project_taxes(
        revenue, costs, num_people, country, "Business", "Mixed", 0
    )
    ...
    sorted_strategies = sorted(
        optimal["all_strategies"], key=lambda x: x["net_income_group"], reverse=True
    )
    ...
    print(
        f"   That's {(optimal['savings'] / optimal['worst']['net_income_group'] * 100):.1f}% more money!"
    )
