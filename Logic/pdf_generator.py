...
def generate_project_pdf(record_data, people_data, filepath="reports/project_report.pdf"):
    ...
    team_table = Table(team_data, colWidths=[1.5*inch, 1.2*inch, 1.3*inch, 1.3*inch, 1.3*inch])
    ...
    footer_text = f"Generated by MoneySplit on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
...
def generate_summary_pdf(records, stats, filepath="reports/summary_report.pdf"):
    ...
    records_data = [['ID', 'Date', 'Country', 'Tax Type', 'Revenue', 'Tax', 'Net Income']]
    for r in records:
        records_data.append([
            str(r[0]),
            r[8][:10],  # date only
    ...
    records_table = Table(records_data, colWidths=[0.5*inch, 1*inch, 1*inch, 1.2*inch, 1.2*inch, 1*inch, 1.2*inch])
